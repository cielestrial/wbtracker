...
function saveData(data) {
    if (typeof (window.Storage) !== "undefined") {
        ...
        let value = `d:${data.date.day}:${data.surveyValues.toString()}:${data.surveyResult}:${data.comment}`;
        // Debug
        ...
        if (!oldData) {
            ...
            if (oldData.includes(valueRange)) {
                ...
                if (oldData.includes(`d:${data.date.day}`)) {
                    localStorage.setItem(key, `${oldData.slice(0, oldData.lastIndexOf(
                            `:d:${data.date.day}:`) + 0)}:${value}`);
                } else {
...
function readDataFromStorage(ID) {
    ...
    for (let i = 0; i < localStorage.length; i++) {
        // If a key with the same ID already exists
        if (localStorage.key(i).charAt(0) === ID
...
function graphData(ID) {
    if (SurveyReports.length > 0) {
        ...
        let title;
        if (ID === "D") {
            title = `${Months[SurveyReports[0].date.month]} ${SurveyReports[0].date.year} Depression Survey`;
        } else if (ID === "A") {
            title = `${Months[SurveyReports[0].date.month]} ${SurveyReports[0].date.year} Anxiety Survey`;
        }
                        ...
                        external: function (context) {
                            ...
                            if (tooltipModel.body) {
                                ...
                                titleLines.forEach(function (title) {
                                    if (title === "1") {
                                        innerHtml += '<tr><th>' + 'Date: ' + title + "st" + '</th></tr>';
                                    } else if (title === "2") {
                                        innerHtml += '<tr><th>' + 'Date: ' + title + "nd" + '</th></tr>';
                                    } else if (title === "3") {
                                        innerHtml += '<tr><th>' + 'Date: ' + title + "rd" + '</th></tr>';
                                    } else {
                                        innerHtml += '<tr><th>' + 'Date: ' + title + "th" + '</th></tr>';
                                    }
                                ...
                                bodyLines.forEach(function (body, i) {
                                    ...
                                    innerHtml += '<tr><td>' + span + body + "<br>" + "Comment: "
                                            + tooltipComment + '</td></tr>';
                                });
                                innerHtml += '</tbody>';
